...
class _banveState extends State<banveghephu> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('THANH TOÁN BÁN VÉ phu'),
        centerTitle: true,
      ),
                            ...
                            onPressed: xacnhan()
                                ? () async {
                                    ...
                                    var resp = await ApiHelper.post(
                                        servicesAPI.API_DonHang+'DonHang/thuc-hien-ban-ve-cho-ghe-phu',
                                        {
                                          'ToaDo': '',
  ...
  bool xacnhan() {
   if (ve.isEmpty || sdt.length<10 || diemxuongObject == null || giave == '0đ' || giave.isEmpty) {
      return false;
    }
    return true;
  }
